...
namespace OpcodesExecutor
{
	...
	public:
		...
		TEST_METHOD(ROR_shifts_value_in_memory_right_and_add_carry_equal_zero)
		{
			int8_t value = 0b01110111;
			uint16_t address = 0x0013;
			mem_[address] = value;
			Assert::AreNotEqual(int8_t(0b00111011), mem_[address]);
			Assert::IsFalse(reg_.PS[static_cast<uint8_t>(nes::cpu::registers::ProcessorStatus::Carry)]);

			ae_.ROR(address);

			Assert::AreEqual(int8_t(0b00111011), mem_[address]);
		}
